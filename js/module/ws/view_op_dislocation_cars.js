...
(function (window) {
    ...
    $.Text_View =
    {
        'default':  //default language: ru
        {
            ...
            'vodlc_mess_error_access_denied': 'Для учетной записи {0} доступ к операции запрещен!',
                ...
                options: {
                    ...
                    fn_click: function (event) {
                        ...
                        if (this.rRW || this.rAdm) {
                            ...
                            this.on_alert.out_warning_message(langView('vodlc_mess_error_access_denied', App.Langs).format(App.AdminInfo ? App.AdminInfo.name : ''));
                        }
                    }.bind(this),
                }
            };
    ...
    // Показать данные 
    view_op_dislocation_cars.prototype.view = function (id_way, access) {
        ...
        if (this.rRO) {
            ...
            var twon_bts = this.twon_opodl.tab_com.obj_t_report.buttons([7, 8, 9]);
            var twfrom_bts = this.twfrom_opodl.tab_com.obj_t_report.buttons([7]);
            if (this.rRW || this.rAdm) {
                twon_bts.enable();
                twfrom_bts.enable();
            } else {
    ...
    view_op_dislocation_cars.prototype.update_from_way = function (id_way, callback) {
        this.confirm_update_way_from(id_way,
            function () { // Ok
                ...
                this.form_from_setup.el.select_id_way_from.val(id_way ? id_way : -1);
                // Запустим паралельно
                var pr_ufw = 1;
    ...
    view_op_dislocation_cars.prototype.validation = function (result) {
        ...
        if (loc1 === loc2) {
            this.form_on_setup.set_element_validation_error('locomotive1', langView('vodlc_mess_error_equal_locomotive', App.Langs), false);
            this.form_on_setup.set_element_validation_error('locomotive2', langView('vodlc_mess_error_equal_locomotive', App.Langs), false);
            valid = false;
        } else {
            ...
            if ((loc2 !== null && loc2 !== '') && result.new && !result.new.datalist_locomotive2) {
                this.form_on_setup.set_element_validation_error('locomotive2', langView('vodlc_mess_error_not_locomotive', App.Langs).format(loc2), false);
                valid = false;
            }
        }
        // Проверим время
        if (result.new && result.new.input_datetime_time_aplly) {
            ...
            if (minutes > 0) {
                this.form_on_setup.set_element_validation_error('time_aplly', langView('vodlc_mess_error_start_time_aplly', App.Langs).format(operation_end), false);
                valid = false;
            }
            var minutes = aplly.diff(curr, 'minutes');
            if (minutes < App.wsd_setup.dislocation_start_dt_min) {
                this.form_on_setup.set_element_validation_error('time_aplly', langView('vodlc_mess_error_min_time_aplly', App.Langs).format(App.wsd_setup.dislocation_start_dt_min * -1), false);
                valid = false;
            }
            if (minutes > App.wsd_setup.dislocation_start_dt_max) {
                this.form_on_setup.set_element_validation_error('time_aplly', langView('vodlc_mess_error_max_time_aplly', App.Langs).format(App.wsd_setup.dislocation_start_dt_max), false);
                valid = false;
            }
        }
        // Проверим состав
    ...
    //
    view_op_dislocation_cars.prototype.clear_all = function () {
        this.form_on_setup.clear_all();
        this.form_from_setup.clear_all();
